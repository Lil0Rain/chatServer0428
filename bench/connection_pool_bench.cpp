...
void worker(int thread_id, int req_per_thread, BenchResult& result) {
	...
	for (int i = 0; i < req_per_thread; ++i) {
		...
		if (conn) {
			...
			while (wait_ms < prev_min && !result.min_wait_ms.compare_exchange_weak(prev_min, wait_ms))
				;

			// Update max atomically
			int64_t prev_max = result.max_wait_ms.load();
			while (wait_ms > prev_max && !result.max_wait_ms.compare_exchange_weak(prev_max, wait_ms))
				;
